...
def _seed_minimal(client: TestClient, token: str):
    ...
    lid_a = r.json()["id"]
    r = client.post("/lists/", json={"name": "List B", "space_id": sid}, headers=headers); assert r.status_code == 200, r.text
    lid_b = r.json()["id"]
    # Tasks
    def mk(name, status, priority, list_id):
        payload = {"name": name, "status": status, "priority": priority, "list_id": list_id, "space_id": sid}
        r2 = client.post("/tasks/", json=payload, headers=headers); assert r2.status_code == 200, r2.text
        return r2.json()["id"]
    ...
    return {"wid": wid, "sid": sid, "lid_a": lid_a, "lid_b": lid_b, "t1": t1, "t2": t2, "t3": t3, "headers": headers}


def test_auth_invalid_token_401(client: TestClient):
    r = client.get("/auth/protected", headers={"Authorization": "Bearer definitely-not-a-valid-token"})
...
def test_task_get_and_delete_404(client: TestClient):
    ...
    assert r.status_code == 401 or r.status_code == 404  # unauth or not found both cover code paths
...
def test_filters_cf_ops_and_grouping(client: TestClient):
    ...
    # Create + enable CF
    r = client.post(f"/workspaces/{wid}/custom-fields",
                    json={"name": "Team", "field_type": "Text"}, headers=headers)
    assert r.status_code == 200, r.text
    cf_id = r.json()["id"]

    r = client.post(f"/lists/{seeded['lid_a']}/custom-fields/{cf_id}/enable", headers=headers)
    assert r.status_code == 200, r.text

    # Set CF values for two tasks (third remains empty)
    r = client.put(f"/tasks/{seeded['t1']}/custom-fields/{cf_id}", json={"value": "Engineering"}, headers=headers); assert r.status_code == 200
    r = client.put(f"/tasks/{seeded['t2']}/custom-fields/{cf_id}", json={"value": "Marketing"},  headers=headers); assert r.status_code == 200
    ...
    r = client.post(f"/workspaces/{wid}/tasks/filter", json=body, headers=headers); assert r.status_code == 200, r.text
