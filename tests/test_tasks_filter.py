...
def _seed_basics(client: TestClient, token: str) -> Dict[str, str]:
    ...
    # Space
    r = client.post("/spaces/", json={"name": "Ops", "workspace_id": wid}, headers=headers)
    assert r.status_code == 200, r.text
    sid = r.json()["id"]
    ...
    r = client.post(f"/tasks/{t1}/tags:assign", json={"tag_ids": [red_id, blue_id]}, headers=headers)
    assert r.status_code == 200, r.text
    r = client.post(f"/tasks/{t2}/tags:assign", json={"tag_ids": [red_id]}, headers=headers)
    assert r.status_code == 200, r.text
    ...
    r = client.put(f"/tasks/{t1}/custom-fields/{cf_id}", json={"value": "Engineering"}, headers=headers)
    assert r.status_code == 200, r.text
    r = client.put(f"/tasks/{t2}/custom-fields/{cf_id}", json={"value": "Marketing"}, headers=headers)
    assert r.status_code == 200, r.text
...
def test_filter_by_status_eq(client: TestClient):
    ...
    body = _filter(client, wid, headers, {
