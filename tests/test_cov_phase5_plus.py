...
def _seed(client: TestClient, token: str):
    ...
    user_id = jwt.decode(token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])["sub"]
    ...
    r = client.post(f"/lists/{lid_a}/custom-fields/{cf_id}/enable", headers=headers); assert r.status_code == 200, r.text

    # Set CF values for A-list tasks only; leave List B without value
    r = client.put(f"/tasks/{t1}/custom-fields/{cf_id}", json={"value": "Engineering"}, headers=headers); assert r.status_code == 200
    r = client.put(f"/tasks/{t2}/custom-fields/{cf_id}", json={"value": "Marketing"},  headers=headers); assert r.status_code == 200

    return {"wid": wid, "sid": sid, "lid_a": lid_a, "lid_b": lid_b, "t1": t1, "t2": t2, "t3": t3, "headers": headers, "user_id": user_id, "cf_id": cf_id}
...
def test_admin_delete_and_move_subtask_invalid_parent(client: TestClient):
    ...
    r = client.post(f"/tasks/{s['t1']}/move", json={"new_parent_task_id": str(uuid4())}, headers=headers)
...
def test_cf_contains_in_and_group_by_cf(client: TestClient):
    ...
    r = client.post(f"/workspaces/{wid}/tasks/filter", json={
    ...
    r = client.post(f"/workspaces/{wid}/tasks/filter", json={
        "scope": {"workspace_id": wid},
        "filters": [{"field": f"cf_{cf}", "op": "in", "value": ["Engineering", "Sales"]}]
    }, headers=headers)
    assert r.status_code == 200
    ids = _ids(r.json())
